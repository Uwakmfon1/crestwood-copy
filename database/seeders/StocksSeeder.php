    ...
    public function run()
    {
        ...
        if ($response->successful()) {
            ...
            foreach ($stocks as $stock) {
                // Filter to include only stocks from NASDAQ and NYSE
                if (in_array($stock['exchange'], ['NASDAQ', 'NYSE'])) {
                    $stockData[] = [
                        'symbol' => $stock['symbol'],
                        'name' => $stock['name'] ?? null, // Handle missing name key
                        'exchange' => $stock['exchange'] ?? 'NASDAQ',
                    ];
                }
            }

            // Check if there are any stocks to proceed with
            if (!empty($stockData)) {
                // Step 2: Fetch company profiles for all filtered US stocks
