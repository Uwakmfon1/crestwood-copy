            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-sm-flex d-block justify-content-between align-items-baseline mb-2">
                        <h6 class="card-title my-2">@if(request()->offsetExists('active')) Active @elseif(request()->offsetExists('pending')) Pending @elseif(request()->offsetExists('cancelled')) Cancelled @elseif(request()->offsetExists('settled')) Settled @endif Savings</h6>
    ...
    <script>
        $(document).ready(function () {
            $('#investmentTable').DataTable({
                ...
                "searching": true,
                "ajax":{
                    "url": "{{ route('admin.savings.ajax', $type) }}",
                    "dataType": "json",
                    "type": "POST",
