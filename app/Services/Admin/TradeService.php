...
class TradeService
{
    ...
    {   
        return view('admin.user.trade.buy', ['user' => $user, 'rate' => ['gold'=>$this->homeService->fetchGoldBuyPriceInNGN(), 'silver' => $this->homeService->fetchSilverBuyPriceInNGN()]]);
    }

    public function sell(User $user)
    {
        return view('admin.user.trade.sell', ['user' => $user, 'rate' => ['gold' => $this->homeService->fetchGoldSellPriceInNGN(), 'silver' => $this->homeService->fetchSilverSellPriceInNGN()]]);
    }
    ...
    public function buyStore(Request $request): RedirectResponse
    {
        ...
        if($request['product'] == 'gold'){
            $gramsToNgn = $this->homeService->fetchGoldBuyPriceInNgn();            
        }elseif($request['product'] == 'silver'){
            $gramsToNgn = $this->homeService->fetchSilverBuyPriceInNgn();            
        }else{
            $gramsToNgn = 0;
        }
        if ($gramsToNgn == 0){
    ...
    public function sellStore(Request $request)
    {
        ...
        if($request['product'] == 'gold'){
            $gramsToNgn = $this->homeService->fetchGoldSellPriceInNGN();            
        }elseif($request['product'] == 'silver'){
            $gramsToNgn = $this->homeService->fetchSilverSellPriceInNGN();            
        }else{
            $gramsToNgn = 0;
        }
        if ($gramsToNgn == 0){
